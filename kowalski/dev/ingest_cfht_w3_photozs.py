...
def process_file(_file, _collection, _batch_size=2048, verbose=False, _dry_run=False):
    ...
    for ii, dff in enumerate(pd.read_csv(_file, chunksize=_batch_size)):
        ...
        # dff['created_at'] = dff['created_at'].apply(lambda x: datetime.datetime.strptime(x, '%Y-%m-%d %H:%M:%S.%f'))
        ...
        for ie, doc in enumerate(batch):
            try:
                ...
                doc['red'] = True if doc['red'] == 't' else False
...
if __name__ == '__main__':
    ...
    _location = '/_tmp/cfht_w3_photozs/'

    files = glob.glob(os.path.join(_location, '*.csv'))

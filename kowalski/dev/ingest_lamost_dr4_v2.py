...
def process_file(_file, _collection, _batch_size=2048, verbose=False, _dry_run=False):
    ...
    for ii, dff in enumerate(pd.read_csv(_file, delimiter='|', chunksize=_batch_size)):

        print(f'{_file}: processing batch # {ii + 1}')

        # dff.rename(index=str, columns={'obsid': '_id'}, inplace=True)
        dff['_id'] = dff['obsid']
...
if __name__ == '__main__':
    ...
    files = glob.glob(os.path.join(_location, 'lamost_dr4_v2.csv'))
